---
id: 1891
date: '2014-04-18T16:11:36-05:00'
author: 'Ron Bowes'
layout: revision
guid: 'https://blog.skullsecurity.org/2014/1879-revision-v1'
permalink: '/?p=1891'
---

Hey folks,

This is my last writeup for [PlaidCTF](http://www.plaidctf.com)! It's a 275-point pwnable level called Kappa, and the goal is to capture a bunch of Pokemon and make them battle each other!
...
When you run [Kappa](https://blogdata.skullsecurity.org/kappa), you get a Pokemon interface:
...
The three important fields are <tt>artwork</tt>, which is a differently sized array for each type, as well as <tt>actions</tt> and <tt>function\_status</tt>. <tt>function\_status</tt> is particular important, because it's a function pointer to the function that displays the Pokemon's status. And function pointers are fun. :)
...
Space, space, single quote, double quote. That looks suspiciously like the ascii art we see above! And, in fact, it is! The <tt>artwork</tt> pointer of the Charizard we just caught overwrote the <tt>function\_status</tt> pointer of the next Pokemon. Then, when we attempted to call <tt>function\_status</tt>, it called some ascii art and crashed.
...
When the Kakuna is replaced by a Charizard, the program still thinks that the Pokemon is a charizard. Then, when it calls fcn\_status(), it's actually calling an address pointer read from the artwork. If you look at the diagram above, you'll see that the same place that Kakuna stores its fcn\_status pointer, Charizard stores its artwork.
...
Where 'edx' is a pointer to the full structure—the name, etc.—and eax is fully controlled. That means we can call a, arbitrary function and pass a string argument as the first parameter. system(), anyone?

The first thing we tried was jumping to the heap. Suffice to say, that failed. So we had to be more clever and us a return-into-libc attack. The problem was, we didn't know where libc was!

Remember earlier when I said that <tt>actions</tt> was an important field? Well, the <tt>actions</tt> field, normally, is a list of the actions that each Pokemon can take. Did you see under Bird Jesus's status where it said <tt>Attack: Gust</tt>? Well, that's reading a string pointed to by our struct. And since we control the struct, we control the artwork!

To put a kink in it, it's actually dereferenced before being read. Which means that whatever we point it to will be dereference then printed. So, it's a pointer to a string pointer. Meaning we can only read memory if we have a pointer to it!
...
To rephrase all that, we can set the Pokemon's action to a pointer to a string pointer. If we use 08048510+2, then we're setting it to a pointer to 0x804AEB4, which in turn is the read() libc call. Then when it tries to print, it's going to print the address of read() (and also printf() and some other functions) as a string, until it reaches a NUL byte!
...
This is where we cheated a bit. And by cheated, I mean use a common CTF technique: We stole a copy of libc from [ezhp](/2014/plaidctf-writeup-for-pwnage-200-a-simple-overflow-bug)'s level!
