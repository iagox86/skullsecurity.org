...
date: '2017-06-13T10:32:19-05:00'
author: 'Ron Bowes'
layout: revision
guid: 'https://blog.skullsecurity.org/2017/2307-revision-v1'
permalink: '/?p=2335'
---

Hey everybody,

A couple months ago, we ran BSides San Francisco CTF. It was fun, and I posted blogs about it at the time, but I wanted to do a late writeup for the level [b-64-b-tuff](https://github.com/BSidesSF/ctf-2017-release/tree/master/pwn/b-64-b-tuff).
...
I don't want to dwell too much on the basics, so I highly recommend reading [PRIMER.md](https://github.com/iagox86/ctfworkshop-2017/blob/master/PRIMER.md), which is a primer on assembly code and shellcode that I recently wrote for a workshop I taught.
...
I originally did this by scripting out to <tt>ndisasm</tt> on the commandline, a tool that we'll see used throughout this blog, but I didn't keep that code. Instead, I'm going to use the [Crabstone Disassembler](https://github.com/bnagy/crabstone), which is Ruby bindings for Capstone:
...
Anyway, running that produces [quite a lot of output](https://gist.github.com/iagox86/262d31185a4695f7255347fc75d000e0). We can feed it through <tt>sort</tt> + <tt>uniq</tt> to get [a much shorter version](https://gist.github.com/iagox86/b96e30393d60381e552718ab904a54ea).

From there, I manually went through the full 2000+ element list to figure out what might actually be useful (since the vast majority were basically identical, that's easier than it sounds). I moved all the good stuff to the top and got rid of the stuff that's useless for writing a decoder stub. That left me with [this list](https://github.com/BSidesSF/ctf-2017-release/blob/master/pwn/b-64-b-tuff/solution/command_set.txt). I left in a bunch of stuff (like multiply instructions) that probably wouldn't be useful, but that I didn't want to completely discount.
...
For the purposes of this blog, we're going to have 12 bytes of shellcode: a simple call to the sys\_exit() syscall, with a return code of 0x41414141. The reason is, it demonstrates all the fundamental concepts (setting variables and making syscalls), and is easy to verify as correct using <tt>strace</tt>
...
If you want to try running it, you can use my [run\_raw\_code.c](https://github.com/BSidesSF/ctf-2017-release/tree/master/forensics/ximage/solution) utility (there are plenty just like it):
...
The read() call is where the run\_raw\_code stub is reading the shellcode file. The 1094795585 in exit() is the 0x41414141 that we gave it. We're going to see that value again and again and again, as we evaluate the correctness of our code, so get used to it!
...
Let's start by looking at how we can clear eax using [our instruction set](https://github.com/BSidesSF/ctf-2017-release/blob/master/pwn/b-64-b-tuff/solution/command_set.txt). We have a few promising instructions for changing eax, but these are the ones I like best:
...
We'll start by trying a pure bruteforce (this code is essentially from [my solution](https://github.com/BSidesSF/ctf-2017-release/blob/master/pwn/b-64-b-tuff/solution/sploit.rb)):
...
The full output is [here](https://gist.github.com/iagox86/0e37a44b13007368d8027bfe7ab5ff32), but the summary is:
...
So if we want to set eax to 0x00000001 (for the sys\_exit syscall), we can simply feed it into this code and convert it to assembly:
...
Then run each of those values through our get\_xor\_values\_32() function from earlier:
...
So now we actually want to alter the shellcode in memory (self-rewriting code!). We need an instruction to do that, so let's look back at the [list of available instructions](https://github.com/BSidesSF/ctf-2017-release/blob/master/pwn/b-64-b-tuff/solution/command_set.txt)! After some searching, I found one that's promising:
...
- Load the encoded shellcode (half of the xor pair, ie, the return value from get\_xor\_values\_32()) into a known memory address (in our case, it's going to be 0x141 bytes after the start of our code)
...
Recall that for our sys\_exit shellcode, the xor pairs we determined were: 0x43614241 ^ 0x2b39432b, 0x6a6a6a6a ^ 0x2b2b2b2b, and 0x6a6a6a62 ^ 0x2b2b2b39.
...
To run this, we have to patch run\_raw\_code.c to load the code to the correct address:
...
The decoder stub and encoded shellcode can be quite easily generated programmatically rather than doing it by hand, which is extremely error prone (it took me 4 tries to get it right - I messed up the start address, I compiled run\_raw\_code in 64-bit mode, and I got the endianness backwards before I finally got it right, which doesn't sound so bad, except that I had to go back and re-write part of this section and re-run most of the commands to get the proper output each time :) ).
...
As usual, we start by looking at our [set of available instructions](https://github.com/BSidesSF/ctf-2017-release/blob/master/pwn/b-64-b-tuff/solution/command_set.txt) to see what we can use to set the most significant bit (let's start calling it the "MSB" to save my fingers).
...
You can take a look at how I do this in [my final code](https://github.com/BSidesSF/ctf-2017-release/blob/master/pwn/b-64-b-tuff/solution/sploit.rb). It's going to be a little different, because instead of using our xor trick to set edx to 0x7F, I instead push 0x7a / pop edx / increment 6 times. The only reason is that I didn't think of the xor trick when I was writing the original code, and I don't want to mess with it now.
...
We can dump allll that code into a file, and run it with run\_raw\_code (don't forget to apply the patch from earlier to change the base address to 0x41410000, and don't forget to compile with -m32 for 32-bit mode):
...
So that, ladies and gentlemen and everyone else, is how to encode some simple shellcode into base64 by hand. [My solution](https://github.com/BSidesSF/ctf-2017-release/blob/master/pwn/b-64-b-tuff/solution/sploit.rb) does almost exactly those steps, but in an automated fashion. I also found a few shortcuts while writing the blog that aren't included in that code.
