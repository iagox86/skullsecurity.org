...
date: '2008-09-02T20:24:13-05:00'
author: 'Ron Bowes'
layout: revision
guid: 'http://www.skullsecurity.org/blog/?p=61'
permalink: '/?p=61'
---
...
- Server responds with its protocol choice, a bunch of its capabilities/settings, and its challenge value (used when creating the login response)

20. SMB\_COM\_SESSION\_SETUP\_ANDX
- Client sends its username, domain, and hashes password(s); the hashes can be in any format the server supports, from plaintext to NTLMv2, the server will check against every possibility.
- Server responds with either the UID (User ID) for the session, or with an error, depending on if a valid username/password was given; also informs the user if their username was incorrect, resulting on them being logged in as 'GUEST'<sup>3</sup>.

22. SMB\_COM\_TREE\_CONNECT\_ANDX
- Client sends the tree it wishes to connect to (eg, "IPC$" for special communications, "C$" for the C drive, "KITTEN" for a shared folder called "KITTEN", etc).
...
<sup>3</sup> That's right -- if you mistype your username, and the remote machine has the 'GUEST' account enabled, you're automatically logged in as the GUEST account.