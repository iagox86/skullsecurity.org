...
date: '2009-02-20T09:53:40-05:00'
author: 'Ron Bowes'
layout: revision
guid: 'http://www.skullsecurity.org/blog/?p=182'
permalink: '/?p=182'
---
...
This is simple and obvious, much like Tip 1, but it's important. Take another look at the screenshots in Tip 1; notice that each login request in mine is 2 packets, and each login in theirs is 4 packets (not counting the other network nonsense). Since all these login packets are roughly the same size, that means half the network traffic. On a slower network or for a long test, that could be significant.

The reason for the extra traffic is that Windows defaults to using extended security negotiations, or NTLMSSP. NTLMSSP is more modern, and is probably the "proper" way of connecting to Windows. However, all versions of Windows, at least up to Vista, still understand the old style, so why not take advantage? The issue here is that, if you're trying to use Windows' libraries to do this, you can't get this level of control (nor can you do the tricks from Tips 1, 3, 4, and maybe others) .

So, what's a programmer to do? InAnd in this case, I mean the SMB protocol. Yes, I realize that implementing the while SMB protocol is a daunting task, but keep in mind that you only need the first two packets, and the first one is static. Not so bad, eh? There are at least three reasons you'll want to do this. One is this tip, and the next two in Tips 3 and 4.
