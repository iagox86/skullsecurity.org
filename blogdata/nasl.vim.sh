#!/bin/sh
# By Ron Bowes <rbowes@tenable.com>
# Created September 23, 2010
#
# Usage: nasl.vim.sh [path_to_plugins]
# 

# If they didn't give an argument, take the standard path
if [ $# -ne 1 ]; then
 DIR='/opt/nessus/lib/nessus/plugins/'
else
 DIR=$@
fi
...
# Get the functions from every .inc file
echo '" Functions extracted from .inc files'
cat $DIR/*.inc | grep '^function' | cut -b10- | sed 's/(.*//' | sed 's/[[:space:]]//' | sort | uniq | sed -r 's/^(.*)$/syn keyword naslFunction \1/'
echo ''

# Get the constants from every .inc file
echo '" Constants extracted from .inc files'
cat $DIR/*.inc | egrep '^[a-zA-Z0-9_]+ = [a-fA-F0-9x]+;' | sed 's/[[:space:]].*//' | sort | uniq | sed -r 's/^(.*)$/syn keyword naslConstant \1/'
echo ''
